...
def handle(spec: FlowSpec, context) -> FlowSpec:
    ...
    if not spec.artifact_id:
        logger.warning("No artifact_id provided; skipping graph fetch")
        return spec
    ...
    try:
        resp = httpx.get(
            f"{bff_url}/app/graphs/{spec.artifact_id}",
        ...
        if resp.status_code == 200:
            data = resp.json()
            import json
            spec.graph_json = json.dumps(data.get("graph", data))
        else:
            logger.warning(f"Graph fetch returned {resp.status_code}: {resp.text[:200]}")
    except Exception as e:
        logger.warning(f"Failed to fetch graph: {e}")
